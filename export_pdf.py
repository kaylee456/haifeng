...
# ---------------------------------------------------------------------------
router = APIRouter()
# project_manager 由外部注册时注入，这里仅声明接口依赖
try:
...
def _write_toc_xsl(xsl_path: str) -> None:
    """
    生成标书风格目录样式表：
    • 一/二/三级标题依次缩进（0 / 20px / 40px）
    • 标题与页码之间虚线填充
    • 目录条目为可点击链接（wkhtmltopdf 自动生成 href="#dest..."）
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          /* 各层级缩进 */
          ul.level-1 { padding-left: 0; }
          ul.level-2 { padding-left: 2em; }
          ul.level-3 { padding-left: 4em; }
          ...
          .toc-dots {
            display: table-cell;
            /* 虚线由 background 模拟，兼容 wkhtmltopdf */
          ...
          /* 层级字体差异 */
          li.l1 > .toc-row { font-weight: bold; font-size: 12pt; margin-top: 6px; }
          li.l2 > .toc-row { font-weight: normal; font-size: 11pt; margin-top: 2px; }
          li.l3 > .toc-row { font-weight: normal; font-size: 10.5pt; margin-top: 1px; color: #222; }
...
<style>
  ...
  h1.sec-h1 {
    ...
    /* wkhtmltopdf 从此 h1 生成 outline 一级条目 */
  }
  h2.sec-h2 {
    font-size: 13pt;
    font-weight: bold;
